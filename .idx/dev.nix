{ pkgs, ... }: {
  # Using the unstable channel for more up-to-date packages for Android development.
  channel = "unstable";

  packages = [
    pkgs.gradle
    pkgs.kotlin
    pkgs.jdk17
    # Keep android-sdk and add android-tools for additional utilities
    pkgs.android-sdk
    pkgs.android-tools
  ];

  env = {
    # Correctly set JAVA_HOME
    JAVA_HOME = "${pkgs.jdk17}";
    # Correctly set ANDROID_HOME and ANDROID_SDK_ROOT
    ANDROID_HOME = "${pkgs.android-sdk}";
    ANDROID_SDK_ROOT = "${pkgs.android-sdk}";
    # Add Android SDK tools to PATH
    PATH = "${pkgs.android-sdk}/cmdline-tools/latest/bin:${pkgs.android-sdk}/platform-tools:$PATH";
  };

  idx = {
    extensions = [
      # Corrected Kotlin extension ID
      "fwcd.kotlin"
      # Alternative Android extension (ms-android.android-pack might not be available)
      "vscjava.vscode-android"
    ];

    workspace = {
      onCreate = {
        # Accept Android SDK licenses using the correct sdkmanager path
        accept-licenses = ''
          yes | ${pkgs.android-sdk}/cmdline-tools/latest/bin/sdkmanager --licenses || true
        '';
        # Install required SDK components INCLUDING NDK
        install-sdk = ''
          ${pkgs.android-sdk}/cmdline-tools/latest/bin/sdkmanager \
            "platform-tools" \
            "platforms;android-34" \
            "build-tools;34.0.0" \
            "ndk;25.1.8937393" || true
        '';
        # Setup gradle wrapper
        gradle-wrapper = ''
          cd app && (./gradlew wrapper || gradle wrapper) || true
        '';
      };
      
      onStart = {
        # The onStart hook is a good place to run dev servers or other tasks that should start with the workspace.
        # For this project, we'll leave it empty for now.
      };
    };
  };
}